...
public class SmaTest {
	...
	public void testSma() throws Exception {
		...
		Assert.assertEquals(init.getStorage().getStockSignal("aapl", "testSma", days.get(aaplIndex + 4).getDate()), init.getStorage().getStockSignal("aapl", "testSma", 4));

		Assert.assertEquals(init.getStorage().getStockSignal("aapl", "testSma", days.get(aaplIndex).getDate()), init.getStorage().getStockSignal("aapl", "testSma", 0));
		...
		final double lastSma = init.getStorage().getStockSignal("aapl", "testSma", lastDay.getDate()).getContent(DoubleSignal.class).getValue();
		Assert.assertEquals(lastSum / 5, lastSma, Settings.doubleEpsilon);

		final double lastSmaClose = init.getStorage().getStockSignal("aapl", "testSmaClose", lastDay.getDate()).getContent(DoubleSignal.class).getValue();
