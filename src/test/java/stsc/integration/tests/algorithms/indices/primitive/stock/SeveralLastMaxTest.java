...
import stsc.algorithms.indices.primitive.stock.SeveralLastMax;
...
public class SeveralLastMaxTest {
	...
	public void testSeveralLastMax() throws Exception {
		...
		final Stock aapl = UnitedFormatStock.readFromUniteFormatFile(TestAlgorithmsHelper.resourceToPath("aa.uf"));
		...
		for (int i = aaplIndex; i < days.size(); ++i) {
			...
			if (!stockInit.getStorage().getStockSignal("aa", "slm", day.getDate()).isPresent()) {
				continue;
			}
